...
/**
 * This class handles the responsibility of remembering zoom/pan of an image, and providing an {@link ImageViewState} to load an image view.
 */
abstract class PanAndZoomStateHolder implements ViewPager.OnPageChangeListener, ImageViewEventListener {
    ...
    public Optional<ImageViewState> getImageViewState() {
        return Optional.ofNullable(lastPanAndZoomState)
                .map(state -> new ImageViewState(state.getScale(), state.getCenterPoint().orElse(new PointF(0, 0)), 0));
    }

    @Override
    public void onError() {
        lastPanAndZoomState = null;
    }
    ...
    private void updateLastPanAndZoomState(final @Nullable Float newScale, final @Nullable PointF newCenter) {
        ...
        final PointF center = Optional.ofNullable(newCenter)//
                .orElse(Optional.ofNullable(lastPanAndZoomState)//
                        .flatMap(PanAndZoomState::getCenterPoint)//
                        .orElse(null));
        onPanOrZoomChanged(new PanAndZoomState(scale, center));
    }
